...
def cleanRawText(text,fileName,folder_by_disease,destination):
    ...
    if(destination == "research_files_clean"):
...
if __name__ == '__main__':
    ...
    classification_data_set_path = r'/mnt/share/classification_data_txt'  # this is where the raw research plans (converted into .txt file) are located
    #classification_data_set_path = r'classification_data_txt'  # this is where the raw research plans (converted into .txt file) are located
    folder_list_by_disease = os.listdir(classification_data_set_path)
    print(folder_list_by_disease)

    for folder_by_disease in folder_list_by_disease:
        file_list = os.listdir(classification_data_set_path + "/" + folder_by_disease)

        #print("file_list",file_list)
        totalFileNumber = len(file_list)
        test_set_number = int(TEST_Data_RATIO * totalFileNumber) + 1

        train_set_number= totalFileNumber - test_set_number
        i = 1

        random.shuffle(file_list)  # shuffle the file_list

        for file in file_list:  #randomly select file from the folder of each disease
            fullFilePath = classification_data_set_path + "/" + folder_by_disease + "/" + file
            content = readfile(fullFilePath).strip()  # read file content
            if(i <= train_set_number):
                cleanRawText(content,file,folder_by_disease,destination="output/research_files_clean")   #goes to train_clean
                i = i+1
            else:
                cleanRawText(content,file,folder_by_disease,destination="output/research_files_clean")     #goes to test_clean
