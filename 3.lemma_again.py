...
def generate_trigram_applications_research_files(bigram_model, trigram_model):
    ...
    if 1 == 1:
        ...
        with open(trigram_again_application_research_files_path, 'w', encoding='utf_8') as f:

            for parsed_review in nlp.pipe(line_review(transformed_application_research_files_path),
                                          batch_size=10000, n_threads=4):
...
def compare_orginal_transformed():
    ...
    for research in it.islice(line_review(transformed_application_research_files_path), 150, 151):
        print(research)

    print(u'----' + u'\n')
    print(u'Transformed:' + u'\n')

    with open(trigram_again_application_research_files_path, encoding='utf_8') as f:
...
if __name__ == '__main__':
    ...
    transformed_application_research_files_path = os.path.join(path + '/output', 'trigram_transformed_applications_all.txt')
    ...
    trigram_again_application_research_files_path = os.path.join(path + '/output', 'trigram_transformed_applications_all_again.txt')



    research_files_catelist = listdir_nohidden(research_files_clean_path)  # access all sub-directories under this folder

    generate_unigram_sentences(transformed_application_research_files_path)
